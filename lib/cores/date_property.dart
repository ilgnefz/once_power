...
Future<void> modifyDate(WidgetRef ref) async {
  ...
  for (FileInfo f in files) {
    if (!await fileExist(f)) {
      errors.add(InfoDetail(file: f.filePath, message: S.current.fileNoExist));
      continue;
    }
...
String? setTimeWin(
  ...
  if (handle == INVALID_HANDLE_VALUE) {
    free(nativePath);
    return S.current.fileNoOpen(nativePath);
  }
  ...
  try {
    ...
    if (SetFileTime(handle, creationPtr, accessPtr, writePtr) == FALSE) {
      return S.current.modifyDateFailInfo(nativePath);
    }
